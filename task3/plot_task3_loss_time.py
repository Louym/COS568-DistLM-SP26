#!/usr/bin/env python3
"""
Visualize Task 3 distributed training (PyTorch DDP):
- Per-rank minibatch loss curves (4 ranks) + their per-step mean loss.
- Breakdown of forward, backward, and communication latency.
- Report global mean loss and timing statistics.

Assumes task3/run_glue.py saved, on rank 0:
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Plot Task 3 (DDP) distributed loss/time curves")
    ap.add_argument(
        "--dir",
        type=Path,
        default=Path("/tmp/RTE_task3"),
        help="Output dir used by task3/run_glue.py",
    )
    ap.add_argument(
        "--out",
        type=Path,
        default=Path(__file__).resolve().parent / "task3_loss_time.png",
    )
    ...
    # top: per-rank loss curves + average loss curve
    ...
    ax0.set_ylabel("Loss")
    ax0.set_title("Task 3 (DDP): per-rank minibatch loss and mean loss")
    ...
    ax1.set_title("Task 3 (DDP): avg forward / backward / communication time per step")
