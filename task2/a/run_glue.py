...
def train(args, train_dataset, model, tokenizer):
    ...
    ws = torch.distributed.get_world_size() if args.local_rank >= 0 and torch.distributed.is_initialized() else 1
    logger.info("  Total train batch size (w. parallel, distributed & accumulation) = %d",
                   args.train_batch_size * args.gradient_accumulation_steps * ws)
    ...
    if getattr(args, "profile", False) and args.local_rank in [-1, 0]:
        ...
        trace_path = os.path.join(args.output_dir, "task2a_profile_rank0.json")
...
def main():
    ...
    parser.add_argument("--local_rank", type=int, default=-1,
                        help="For distributed training: rank 0..world_size-1. Use -1 for single-node.")
    parser.add_argument("--master_ip", type=str, default="127.0.0.1",
                        help="Master node IP for init_method tcp:// (required for multi-node).")
    ...
    if args.local_rank >= 0:
        ...
        if args.n_gpu > 0:
            torch.cuda.set_device(args.local_rank % args.n_gpu)
